/* TCP BBRx congestion control
 *
 * BBRx is an out-of-tree DKMS TCP congestion-control module derived from
 * Linux TCP BBR. It keeps BBR-style STARTUP, DRAIN, PROBE_BW, and PROBE_RTT,
 * but uses more aggressive experimental parameters than BBRv1.
 *
 *   bottleneck_bandwidth = windowed_max(delivered / elapsed, 10 rounds)
 *   min_rtt              = windowed_min(rtt, 600 seconds)
 *   pacing_rate          = pacing_gain * bottleneck_bandwidth, plus 5%
 *   cwnd                 = cwnd_gain * bottleneck_bandwidth * min_rtt,
 *                          floored at 200 packets
 *
 ...
 *
 * This variant keeps PROBE_RTT. When the min_rtt filter expires on a
 * continuously busy flow, it temporarily caps cwnd to 200 packets for at
 * least 100 ms and one packet-timed round, then returns to STARTUP or
 * PROBE_BW.
 *
 ...
 *   include/net/tcp.h and passes BBRN_TCP_CA_* feature macros.
 */
...
static inline u32 bbrn_tcp_snd_cwnd(const struct tcp_sock *tp)
{
	return tp->snd_cwnd;
}

static inline void bbrn_tcp_snd_cwnd_set(struct tcp_sock *tp, u32 val)
{
...
#define tcp_snd_cwnd(tp) bbrn_tcp_snd_cwnd(tp)
#define tcp_snd_cwnd_set(tp, val) bbrn_tcp_snd_cwnd_set(tp, val)
#endif

static inline u32 bbrn_random_u32_below(u32 ceil)
{
...
/* BBRx uses these top-level modes. */
enum bbr_mode {
...
struct bbr {
	...
	u32     mode:3,		     /* current bbr_mode in state machine */
		...
		probe_rtt_round_done:1,  /* a BBR_PROBE_RTT round at cwnd floor? */
		unused:13,
...
/* BBRx keeps the BBRv1 DRAIN pacing gain. This is not the reciprocal of
 * BBRx's experimental STARTUP gain.
 */
static const int bbr_drain_gain = BBR_UNIT * 1000 / 2885;
/* BBRx uses an experimental cwnd gain of 4.0. */
...
/* BBRx uses an experimental cwnd floor of 200 packets. This is also the
 * cwnd cap while BBRx is in PROBE_RTT.
 */
...
static const u32 bbr_extra_acked_max_us = 50 * 1000;
...
#if defined(BBRN_TCP_CA_HAS_TSO_SEGS)
#if defined(BBRN_TCP_CA_TSO_SEGS_1_ARG)
...
#if (defined(BBRN_TCP_CA_HAS_CWND_EVENT) && defined(BBRN_TCP_HAS_CA_EVENT_TX_START)) || 	!defined(BBRN_TCP_CA_PROBED)
...
#if defined(BBRN_TCP_CA_HAS_CWND_EVENT_TX_START) || 	(!defined(BBRN_TCP_CA_PROBED) && 	 LINUX_VERSION_CODE >= KERNEL_VERSION(7, 1, 0))
...
static void bbr_reset_probe_bw_mode(struct sock *sk)
{
	...
	bbr->cycle_idx = CYCLE_LEN - 1 - bbrn_random_u32_below(bbr_cycle_rand);
 ...
 * BBRx's constants, that clamp is 25 ms and the max filter is an approximate
 * sliding window of 15-30 packet-timed round trips.
 */
...
/* This variant keeps BBR-style PROBE_RTT. If a continuously busy flow has not
 * refreshed its min_rtt estimate within bbr_min_rtt_win_sec, temporarily cap
 * cwnd to bbr_cwnd_min_target for at least bbr_probe_rtt_mode_ms and one
 * packet-timed round, then resume STARTUP or PROBE_BW.
 */
static void bbr_update_min_rtt(struct sock *sk, const struct rate_sample *rs)
{
	...
	if (bbr->mode == BBR_PROBE_RTT) {
		...
		/* Maintain min packets in flight for max(100 ms, 1 round). */
		if (!bbr->probe_rtt_done_stamp &&
 ...
 * The Makefile probes include/net/tcp.h and passes BBRN_TCP_CA_* feature macros.
 */
#if defined(BBRN_TCP_CA_CONG_CONTROL_4_ARGS) || 	(!defined(BBRN_TCP_CA_PROBED) && 	 LINUX_VERSION_CODE >= KERNEL_VERSION(6, 10, 0))
...
static struct tcp_congestion_ops tcp_bbrn_cong_ops __read_mostly = {
...
#elif defined(BBRN_TCP_CA_HAS_MIN_TSO_SEGS) || !defined(BBRN_TCP_CA_PROBED)
...
static int __init bbrn_register(void)
{
	BUILD_BUG_ON(sizeof(struct bbr) > ICSK_CA_PRIV_SIZE);
	return tcp_register_congestion_control(&tcp_bbrn_cong_ops);
}

static void __exit bbrn_unregister(void)
{
	tcp_unregister_congestion_control(&tcp_bbrn_cong_ops);
}

module_init(bbrn_register);
module_exit(bbrn_unregister);
...
MODULE_DESCRIPTION("TCP BBRn (BBR variant for DKMS) Date: 2026-07-11 20:31:02");